...
public class TumbuhanFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        rvEndemik = view.findViewById(R.id.rvTumbuhan);
    ...
    private void setupFilters() {
        repository.getUniqueAsalByTipe("Tumbuhan").observe(getViewLifecycleOwner(), asals -> {
    ...
    private void updateData() {
        if (currentSelectedAsal.equals("Semua")) {
            repository.getEndemikByTipeLimited("Tumbuhan").observe(getViewLifecycleOwner(), endemiks -> {
                adapter.setData(endemiks);
            });
        } else {
            repository.getEndemikByTipeAndAsal("Tumbuhan", currentSelectedAsal).observe(getViewLifecycleOwner(), endemiks -> {
                adapter.setData(endemiks);
            });
        }
    }
